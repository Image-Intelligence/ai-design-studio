...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (isNaN(messageId) || !stepId || !operations || operations.length === 0 || operations.length > 20) {
    return jsonPrivate({ error: 'messageId, stepId and 1-20 operations are required' }, { status: 400 })
  }
  ...
  if (!step?.editRecipe) {
    return jsonPrivate({ error: 'This image has no editable layer recipe' }, { status: 400 })
  }
  ...
  if ('error' in out) return jsonPrivate({ error: out.error }, { status: 400 })
  ...
  return jsonPrivate({ imageUrl: out.imageUrl, width: out.width, height: out.height })
}
